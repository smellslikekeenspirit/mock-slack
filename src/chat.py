...
def create_user(user_id: int, name: str, phone_number: int, email: str,
                suspended_since: None, suspended_till: None) -> str:
    """
    Creates a user
    ...
    string = f'INSERT INTO users(user_id, name, phone_number, email, suspended_since, suspended_till)' \
             ...
             f'\'{email}\', {suspended_since}, {suspended_till})'
    exec_commit(string)
...
def change_username(user_id, new_name, change_time=None):
    """
    changes username if it has not been changed in the last six months
    :param user_id:
    :param new_name:
    :param change_time:
    :return:
    ...
    change_date_list = exec_get_all('SELECT username_change_date FROM users WHERE email = %s', (email,))
    ...
    if not change_date or change_date + six_months_later <= new_time:
        exec_commit('UPDATE users SET username_change_date = %s WHERE email = %s', (new_time, email))
        exec_commit('UPDATE users SET username = %s WHERE email = %s', (new_name, email))
        return "User successfully changed username to " + new_name
    return "User changed their username in the last 6 months"


def create_message(message_id: int, sender_id: int,
                   receiver_id: int, time_sent: None, message: str) -> str:
    """
    ...
    suspension_dates = exec_get_all('SELECT suspended_start, suspended_end FROM users WHERE email = %s',
                                    (sender_email,))
    ...
    exec_commit(f'INSERT INTO users(message_id, sender_id, receiver_id, time_sent, message, is_read)'
                f' VALUES (%s,%s,%s,%s)',
                (message_id, sender_id, receiver_id, init_time, message, False))
...
def read_message(message_id, receiver_id):
    ...
    :return: text content
    """
    texts = exec_get_all('SELECT text FROM messages WHERE message_id = %s AND receiver_id = %s',
                             (message_id, receiver_id))
    if len(texts) == 1:
        exec_commit('UPDATE messages SET is_read = TRUE WHERE id = %s', (message_id,))
        return texts[0][0]
...
def get_unread_messages(receiver_id):
    ...
    if not user_exists(get_email_by_id(receiver_id)):
        return []
    unreads = exec_get_all('SELECT id FROM messages WHERE is_read = FALSE'
                           'AND receiver_id = %s', (receiver_id,))
    return unreads
...
def get_messages_from(receiver_id, sender_id):
    ...
    if not user_exists(receiver_email):
        return "User" + receiver_email + "doesn't exist"
    return exec_get_all('SELECT id FROM messages WHERE sender_id = %s AND receiver_id = %s',
...
def suspend_user(email, end_suspension, start_suspension=None):
    ...
    exec_commit('UPDATE users SET suspended_start = %s AND suspended_end = %s WHERE email = %s',
...
def resume_user(email):
    ...
    :return:
    """
    exec_commit('UPDATE users SET suspended_start = NULL AND suspended_end = NULL WHERE email = %s', email)
